...
def compose_text(title, url):
    ...
    # 自動ハッシュタグ生成（タイトルから名詞抽出簡易）
    tags = []
    words = re.findall(r'\w+', title)
    for w in words:
        if len(w) > 2 and len(tags) < 3:  # 簡易フィルタ
            tags.append(f"#{w}")
    hashtags = " ".join(tags)
    ...
    try:
        if image_url:
            ...
            with open("/tmp/temp_image.jpg", "wb") as f:
                f.write(r.content)
            # v2 API 用にアップロード
            media = client.media_upload(filename="/tmp/temp_image.jpg")
...
def main():
    ...
    if os.getenv("TWITTER_DRY_RUN") == "1":
        ...
        logging.info("Sleeping %d seconds before posting", delay)
        time.sleep(delay)
